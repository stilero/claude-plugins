...
- Test comments that misdescribe what the test does or how it achieves isolation — when a test has a comment explaining *why* it hits a certain endpoint, uses a certain path, or avoids interference with other tests, verify the claim against the actual code. Common lies: "hits the health endpoint (excluded from rate limiting)" when the test actually hits a regular endpoint; "unique path avoids interference" when the middleware keys on IP, not path; "this endpoint is not rate-limited" when no exclusion exists in the middleware config. For every explanatory comment in a test, trace the claim: read the middleware/config to confirm exclusions, check what the rate-limit/cache key actually is, and compare the comment's isolation theory against the real keying strategy. A wrong comment is worse than no comment — it stops the next developer from questioning the broken isolation
