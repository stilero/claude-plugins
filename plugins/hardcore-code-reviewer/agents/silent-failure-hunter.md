...
**Non-throwing last-resort handlers**
- Code inside `process.on('uncaughtException')`, `process.on('unhandledRejection')`, `process.on('SIGTERM'/'SIGINT')`, bootstrap crash loggers, and any handler whose contract is "this is the last chance to act before the process dies" must be non-throwing. If an operation inside these handlers throws (or rejects without a catch), the handler itself fails — potentially leaving the process alive after an `uncaughtException` (because the scheduled exit never runs), double-faulting an `unhandledRejection` into an `uncaughtException`, or silently skipping the structured crash log that ops relies on for post-mortem.
  - **Logger calls can throw.** `logger.fatal(obj)` / `logger.error(obj)` typically runs the object through a serializer (Pino's `serializers`, Winston's `format`). If serialization throws — circular reference, getter that throws, BigInt, Symbol, or a serializer bug — the log call throws, and any code after it (like `exitAfterFatalLog()` or `process.exit()`) never executes. Wrap logger calls in last-resort handlers with `try/catch` and ensure the exit/cleanup runs in a `finally` block (or before the log attempt with a `setTimeout` safety net).
  - **`JSON.stringify` can throw.** Circular structures, BigInt values, objects with throwing `.toJSON()`, and certain proxy traps all cause `JSON.stringify` to throw. In a bootstrap crash logger or manual structured-log emitter that uses `JSON.stringify`, an unguarded call can prevent the crash log from being emitted AND can prevent the subsequent `process.exit` or exit scheduling from executing. Guard with `try/catch` and fall back to a minimal, always-serializable payload (`{ message: String(err), fatal: true }` or similar).
  - **General pattern**: every operation in a last-resort handler should be individually wrapped or the entire handler body should be in a `try { ... } finally { exit() }`. The `finally` is critical — it ensures the exit path runs even when the log/cleanup throws. Never assume that logger calls, serialization, file I/O, or metric emission inside crash handlers are safe — they run in a degraded context where the original error may have left shared state (buffers, streams, singletons) in an inconsistent state.
  - Severity: BLOCKING when the handler's failure can leave the process alive after a fatal error or prevent the intended crash log/metric from being emitted.
