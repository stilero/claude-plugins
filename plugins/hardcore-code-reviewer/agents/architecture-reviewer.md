...
- **Behavior/schema/decoration added to one consumer of a shared mechanism but not its siblings.** When the diff attaches a behavior, schema, decorator, middleware option, validator, or contract-shaping piece (response schema, rate-limit options, auth requirement, audit log call, metric tag, header decoration) to one route, handler, controller, or job that uses a shared mechanism — and other consumers of the same mechanism exist — the result is a partial/inconsistent application that drifts the codebase. Worked example: a `TooManyRequestsSchema` (429 response schema) attached to the purchases routes that use `sensitiveRouteRateLimit()`, while the other routes using the same `sensitiveRouteRateLimit()` middleware (`features/users/routes/users.routes.ts:42`, `integrations/cloudinary/routes/cloudinary.routes.ts:70` and `:86`) are left untouched — the generated Swagger documents 429 for one set of rate-limited routes and not the other, even though the rate-limit behavior is identical. How to find: when the diff adds anything that decorates, documents, or shapes the contract of a route/handler that uses a shared helper or middleware, grep the codebase for every other call site of that helper or middleware and check whether the same decoration should propagate. The mental model is "what other rows in the table now have an inconsistent column?" Severity: IMPORTANT for documentation/Swagger inconsistency; BLOCKING when the missing decoration is a security/auth requirement, a validation that prevents an exploit, or an audit hook required for compliance. Fix is usually to apply the decoration consistently across siblings, or to factor the decoration into a wrapper that all sites share so future siblings inherit automatically.
...
6. When the diff calls an API client, SDK, or shared utility, grep for all other call sites of that same function/method and verify the new usage matches existing argument formatting (path prefixes, string templates, option shapes). Then go further: when the diff adds a decoration *around* one call site of a shared mechanism — a response schema attached to a route that uses a shared rate-limit/auth middleware, an audit log call before/after a shared persistence helper, a metric tag on one consumer of a shared queue producer, a validator on one route of a shared registrar — grep every other call site of the underlying mechanism and check whether the same decoration should propagate. Inconsistent decoration of identical shared behavior is itself an architecture issue, separate from argument-formatting drift.
