...
- **Initialization ordering: reading env/config state before it's populated.** When a startup function reads `process.env.X`, a config object, a singleton, or any shared state *before* the initialization step that populates it has run, the read silently gets the pre-init value (`undefined`, a fallback like `'unknown'`, or a stale default). Classic offender: a bootstrap/logger setup function that reads `process.env.SERVICE_VERSION` and runs before `dotenv.config()` / `dotenv-expand` / the config loader — the field falls through to `'unknown'` locally even though `.env` provides the value. The bug is invisible in production (where env vars are injected by the platform) but produces wrong values in local dev, and can also affect crash-handler base fields, structured log attribution, and version stamps on startup diagnostics. How to check: when the diff adds or modifies a function that reads env vars or config at module/startup scope, trace backwards to confirm the loading step (`dotenv.config()`, `config.load()`, `dependencies()`, or equivalent) runs *before* the read. If the read is in a top-level `const` or in a function called before the loader, the value is stale. Fix: either move the env/config loading earlier (before the first consumer) or defer the read to after initialization (lazy getter, function call instead of module-scope `const`). Severity: IMPORTANT (BLOCKING when the stale value is baked into crash-handler fields or log base fields that ops relies on for incident attribution).
