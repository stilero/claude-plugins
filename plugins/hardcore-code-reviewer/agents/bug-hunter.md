...
- **PR description or comment claims that contradict the data in the same file.** When the PR body or a code comment makes a scope claim like "all X are outside Y", "only touches Z", "no entries under features/*", or "baseline contains only non-feature cycles", enumerate the relevant constants/arrays/baselines in the file and verify the claim line-by-line. A single counterexample in `KNOWN_CYCLES`, `ALLOWLIST`, `IGNORED_PATHS`, etc. is a BLOCKING issue — either the claim is wrong or the scope/filter needs to be adjusted. If you were given the PR title and body as context, treat every factual assertion in it as something to verify against the diff.
